...
# Board animation
#plt.switch_backend('tkagg'); # esoteric stuff. switches plotting backend or something. NOT NECESSARY :D new fix
...
# kick off animation
ani = animation.FuncAnimation(fig, updatefig, frames=range(numTurns), interval=1000./fps, blit=False); # blit=False fixes mac crashing issue (redraw whole figure, not just parts that have changed)
plt.show();
...
#Kick off animation
anim = animation.FuncAnimation(fig, animate, init_func=init, frames=range(len(tm)-360), interval=1, blit=False)
